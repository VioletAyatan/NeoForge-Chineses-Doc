...
可扩展 Enum 是对特定原版 enum 的增强，允许向其中添加新条目。其实现方式是在运行时修改 enum 已编译的字节码，从而加入新元素。

## `IExtensibleEnum`

所有可以添加新条目的 enum 都实现了 `IExtensibleEnum` 接口。该接口起到标记作用，使 `RuntimeEnumExtender` 启动插件服务能够识别应转换哪些 enum。

:::warning
你**不应**让自己的 enum 实现此接口。请根据用例改用 Map 或 Registry。  
由于各转换器的执行顺序，没有经过补丁以实现该接口的 enum，无法通过 Mixin 或 Coremod 添加这个接口。
:::

### 创建 Enum 条目

要创建新的 enum 条目，需要创建一个 JSON 文件，并在 `neoforge.mods.toml` 的 `[[mods]]` 块中通过 `enumExtensions` 条目引用它。指定路径必须相对于 `resources` 目录：
...
条目定义由目标 enum 的类名、新字段名称（必须以模组 ID 为前缀）、用于构造条目的构造器描述符，以及要传给该构造器的参数组成。
...
如果某个构造器带有 `@ReservedConstructor` 注解，则不能使用它构造模组 enum 常量。
...
- 引用模组中某个类的 `EnumProxy<TheEnum>` 类型字段（参见上面的 `EnumProxy` 示例）
    - 第一个参数指定目标 enum，后续参数则会传给 enum 构造器
...
- 如果参数是与 enum 上 `@IndexedEnum` 注解相关的 int ID 参数，则会忽略该参数，并以条目的 ordinal 替代。如果该参数以内联方式写在 JSON 中，就必须指定为 `-1`，否则会抛出异常。
- 如果参数是与 enum 上 `@NamedEnum` 注解相关的 String 名称参数，就必须按照 `Identifier` 所使用的 `namespace:path` 格式，以模组 ID 作为前缀，否则会抛出异常。

#### 获取生成的常量

可以通过 `TheEnum.valueOf(String)` 获取生成的 enum 常量。如果使用字段引用提供参数，也可以通过 `EnumProxy#getValue()` 从 `EnumProxy` 对象获取该常量。

## 为 NeoForge 做贡献

要向 NeoForge 添加新的可扩展 enum，至少需要完成以下两项工作：

- 让 enum 实现 `IExtensibleEnum`，以标记应由 `RuntimeEnumExtender` 转换此 enum。
- 添加一个返回 `ExtensionInfo.nonExtended(TheEnum.class)` 的 `getExtensionInfo` 方法。

根据 enum 的具体情况，还需要采取其他措施：

- 如果 enum 有一个应与条目 ordinal 相匹配的 int ID 参数，则应使用 `@IndexedEnum` 为 enum 添加注解；如果 ID 不是第一个参数，还需以 ID 参数的索引作为注解值
- 如果 enum 有一个用于序列化、因此应带有命名空间的 String 名称参数，则应使用 `@NamedEnum` 为 enum 添加注解；如果名称不是第一个参数，还需以名称参数的索引作为注解值
- 如果 enum 会通过网络发送，则应添加 `@NetworkedEnum` 注解，并通过注解参数指定允许在哪个方向发送值（clientbound、serverbound 或 bidirectional）
- 如果 enum 中存在模组无法使用的构造器（例如它们要求提供 Registry 对象，而该 enum 可能在模组注册开始前就已初始化），就应为这些构造器添加 `@ReservedConstructor` 注解

:::note
如果确实有任何条目被添加到 enum 中，`getExtensionInfo` 方法会在运行时经过转换，提供动态生成的 `ExtensionInfo`。
:::
