...
添加数据最简单的方式，是在 `Entity` 类中添加字段。之后可以按任意方式与这些数据交互。然而，一旦你需要同步一些数据，这种方式很快就会变得麻烦。原因在于，大多数实体逻辑只在服务端运行，而更新只会偶尔（取决于 [`EntityType`][entitytype] 的 `clientUpdateInterval` 值）发送到客户端；这就是为什么当服务端 tick 速度过慢时，很容易观察到实体“卡顿”。

因此，原版引入了几个辅助系统，每个系统都有特定用途。如有必要，你也可以选择[发送自定义数据][custom]。
