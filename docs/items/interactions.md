...
命中结果可以是 `HitResult.Type` 枚举所表示的三种类型之一：`BLOCK`、`ENTITY` 或 `MISS`。`BLOCK` 类型的 `HitResult` 可强制转换为 `BlockHitResult`，`ENTITY` 类型的 `HitResult` 可强制转换为 `EntityHitResult`；

两种类型都会提供有关命中的 [方块][block] 或 [实体][entity] 的额外上下文。如果类型为 `MISS`，表示既未命中 `Block`，也未命中 `Entity`，不应强制转换为任何一种子类。
...
- 根据你正在注视的对象（使用 `Minecraft` 中的 [`HitResult`][hitresult]），会发生不同情况：
  - 如果持有带有某个 `DataComponents#PIERCING_WEAPON` 的物品：
    - 仅服务端：调用 `PiercingWeapon#attack`。
      - 使用 `ProjectileUtil#getHitEntitiesAlong` 获取满足以下条件的所有实体：
        - 位于攻击者交互距离与物品攻击距离（`DataComponents#ATTACK_RANGE`）内。
        - 并非无敌，并且可被投射物命中。
        - 不是同一载具的乘客。
      - 对每个实体调用 `LivingEntity#stabAttack`；如果目标成功受到伤害、被击退或被迫下乘，返回 true：
  ...
  - 如果正在注视触及范围内的 [实体][entity]：
    ...
    - 将实体基础伤害（`minecraft:attack_damage` [属性][attribute] 的值）与附魔加成伤害分别计算为两个 float。如果两者都为 0，流程结束。
      - 请注意，这不包括主手物品的 [属性修饰符][attributemodifier]；它们会在检查后添加。
    - 将主手物品的 `minecraft:attack_damage` 属性修饰符添加到基础伤害。
    ...
    - 触发 `SweepAttackEvent`。如果事件的 `isSweeping` 方法返回 true，玩家会执行横扫攻击。默认情况下，它会检查攻击冷却是否 > 90%、攻击是否并非暴击、玩家是否在地面上，以及移动速度是否未超过其 `minecraft:movement_speed` 属性值。
    ...
    - 玩家根据 `SweepAttackEvent#isSweeping` 对附近的 `LivingEntity` 执行横扫攻击。
      - 在该方法中，如果实体位于玩家触及范围内且 `Entity#hurtServer` 返回 true，则再次调用 `LivingEntity#knockback`，进而再次触发 `LivingKnockBackEvent`。
...
- 根据你正在注视的对象（使用 `Minecraft` 中的 [`HitResult`][hitresult]），会发生不同情况：
  - 如果正在注视触及范围内且未超出世界边界的 [实体][entity]：
    - 触发 `PlayerInteractEvent.EntityInteractSpecific`。如果事件被取消，流程结束。
    - **对你正在注视的实体** 调用 `Entity#interactAt`。如果返回确定结果，流程结束。
      - 要为自己的实体添加行为，请重写此方法。要为原版实体添加行为，请使用事件。
    - 如果实体打开界面（例如村民交易 GUI 或运输矿车 GUI），流程结束。
    - 触发 `PlayerInteractEvent.EntityInteract`。如果事件被取消，流程结束。
    - **对你正在注视的实体** 调用 `Entity#interact`。如果返回确定结果，流程结束。
      - 要为自己的实体添加行为，请重写此方法。要为原版实体添加行为，请使用事件。
    ...
    - 如果正在注视的实体是 `LivingEntity`，则对主手 `ItemStack` 调用 `Item#interactLivingEntity`。如果返回确定结果，流程结束。
  - 如果正在注视触及范围内且未超出世界边界的 [方块][block]：
    - 触发 `PlayerInteractEvent.RightClickBlock`。如果事件被取消，流程结束。也可以在此事件中只明确拒绝使用方块或物品。
...
`InteractionResult` 是密封接口，表示物品或空手与某个对象（例如实体、方块等）之间交互的结果。该接口分为四个 record，共有六种可能的默认状态。

首先是 `InteractionResult.Success`，表示操作应视为成功，并结束流程。成功状态有两个参数：`SwingSource` 表示 Entity 是否应在相应[逻辑端][side]挥手；`InteractionResult.ItemContext` 保存交互是否由手持物品引起，以及手持物品使用后转变成什么。挥手来源由以下某个默认状态决定：`InteractionResult#SUCCESS` 表示客户端挥手，`InteractionResult#SUCCESS_SERVER` 表示服务端挥手，`InteractionResult#CONSUME` 表示不挥手。如果 `ItemStack` 发生变化，通过 `Success#heldItemTransformedTo` 设置物品上下文；如果手持物品与对象之间没有交互，则通过 `withoutItem` 设置。默认表示发生了物品交互，但物品没有转变。
...
`Item#use` 的默认实现在物品可食用（具有 `DataComponents#CONSUMABLE`）且玩家能够食用（由于饥饿，或物品始终可食用）时返回 `InteractionResult#CONSUME`；

在物品可食用（具有 `DataComponents#CONSUMABLE`）但玩家不能食用时返回 `InteractionResult#FAIL`。

如果物品可装备（具有 `DataComponents#EQUIPPABLE`），那么换装成功时返回 `InteractionResult#SUCCESS`，并用换下的物品替换手持物品（通过 `heldItemTransformedTo`）；

如果盔甲上的附魔具有 `EnchantmentEffectComponents#PREVENT_ARMOR_CHANGE` 组件，则返回 `InteractionResult#FAIL`。

如果物品可格挡攻击（具有 `DataComponents#BLOCKS_ATTACKS`），则在返回 `InteractionResult#CONSUME` 前调用 `Item#startUsingItem`。否则返回 `InteractionResult#PASS`。
...
- 根据你正在注视的对象（使用 `Minecraft.getInstance().hitResult` 中的 `HitResult`），会发生不同情况：
  ...
  - 如果正在注视触及范围内的 [方块][block]：
    ...
    - 如果按住Ctrl键、玩家处于创造模式且目标方块具有 [`BlockEntity`][blockentity]：
