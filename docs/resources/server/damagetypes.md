...
可以说，伤害类型是伤害来源的模板。换句话说，可以把伤害来源看作伤害类型的实例。伤害类型在代码中以 [`ResourceKey`][rk] 的形式存在，但其全部属性都在数据包中定义。另一方面，伤害来源会由游戏根据数据包文件中的值按需创建，并且可以携带额外的上下文，例如发起攻击的实体。

## 创建伤害类型

首先，需要创建自己的 `DamageType`。`DamageType` 属于[数据包注册表][dr]，因此新的 `DamageType` 不在代码中注册，而会在添加相应文件时自动注册。不过，我们仍需要为代码提供一个获取伤害来源的入口。为此，需要指定一个[资源键][rk]：
...
`DamageSource` 通常会在调用 [`Entity#hurt`][entityhurt] 时即时创建。请注意，由于伤害类型属于[数据包注册表][dr]，因此需要通过 `RegistryAccess` 查询它们；`RegistryAccess` 可通过 `Level#registryAccess` 获取。要创建 `DamageSource`，请调用 `DamageSource` 构造器，它最多接受四个参数：
...
DamageSource damageSource = new DamageSource(
        ...
        // （=上面的参数），箭将是直接实体（=此参数）。类似于
        // 致因实体，此参数并不总是适用，因此可为空。可选，默认为 null。
        null,
        // 造成伤害的实体。该值并非始终适用（例如掉出世界时）
        // ，因此可能是 null。可选，默认为 null。
        null,
...
:::warning
`DamageSources#source` 是对 `new DamageSource` 的包装，但它会交换第二和第三个参数（直接实体与致因实体）的位置。请确保向每个参数传入正确的值。
:::

如果 `DamageSource` 完全不包含实体或位置上下文，可以将其缓存在字段中。对于确实包含实体或位置上下文的 `DamageSource`，通常会像下面这样添加辅助方法：
...
:::tip
原版的 `DamageSource` 工厂位于 `DamageSources` 中，原版的 `DamageType` 资源键位于 `DamageTypes` 中。实体还提供了 `Entity#damageSources` 方法，用于便捷地获取 `DamageSources` 实例。
:::

伤害来源最主要的用途是 `Entity#hurt`。每当实体受到伤害时都会调用该方法。要使用自定义伤害类型伤害某个实体，只需自行调用 `Entity#hurt`：
...
_更多信息请参阅[数据包注册表的数据生成][drdatagen]。_

伤害类型 JSON 文件可以通过[数据生成][datagen]创建。由于伤害类型属于数据包注册表，因此我们通过 `GatherDataEvent#createDatapackRegistryObjects` 添加 `DatapackBuiltinEntriesProvider`，并将自己的伤害类型放入 `RegistrySetBuilder`：
