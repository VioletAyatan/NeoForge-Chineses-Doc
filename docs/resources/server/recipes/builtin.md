...
一些最重要的配方——例如工作台、木棍和大多数工具——都通过有序配方创建。这些配方由合成图案或形状定义（因此称为“有序”），Item 必须按此方式放入。示例如下：
...
- `key` 与 `pattern`：二者共同定义 Item 必须如何放入合成网格。
    - pattern 最多定义三行、每行最多三个字符的字符串，用以描述形状。所有行长度必须相同，即图案必须形成矩形。空格表示应保持为空的槽位。
    - key 将图案中使用的字符与 [Ingredient][ingredient] 关联。在上述示例中，图案中的所有 `X` 必须是铁锭，所有 `#` 必须是木棍。
...
与有序合成配方不同，无序合成配方不关心 Ingredient 的传入顺序。因此它没有 pattern 和 key，只有 Ingredient 列表：
...
- `ingredients`：[Ingredient][ingredient] 列表。出于配方查看需要，代码会保留列表顺序，但配方本身接受任意顺序的 Ingredient。
- `result`：配方结果，即 [ItemStack 模板的 JSON 表示形式][itemjson]。
- 示例中未显示 `group` 键。这个可选字符串属性会在配方书中创建分组；同一组中的配方会合并显示为一个。
...
:::info
为遵循原版标准，单 Item 配方（例如拆分存储 Block）应使用无序配方。
:::

### 灌注 Item

灌注配方是一种特殊的单 Item 合成配方，会将材料 ItemStack 的药水内容复制到结果 ItemStack。例如：
...
- `source`：包含待灌注药水内容的 [Ingredient][ingredient]。
- `material`：用于灌注 source 的 [Ingredient][ingredient]。
- `result`：配方结果，即 [ItemStack 模板的 JSON 表示形式][itemjson]。
...
转化配方是一种特殊的单 Item 合成配方，会将输入 ItemStack 的数据组件完整复制到结果 ItemStack。转化通常发生在两个不同 Item 之间，其中一个是另一个的染色版本。例如：
...
- `category`：可选字段，定义配方书中的分类。
- `group`：可选字符串属性，在配方书中创建分组。同一组中的配方会合并显示为一个，这通常适合转化配方。
- `input`：要转化的 [Ingredient][ingredient]。
- `material`：将 ItemStack 转换为结果的 [Ingredient][ingredient]。
- `material_count`：将 ItemStack 转换为结果所需的 `material` 数量。
- `result`：配方结果，即 [ItemStack 模板的 JSON 表示形式][itemjson]。
- `add_material_count_to_result`：是否将所用材料数量加到结果返回的 Item 数量中。通常用于复制 Item 数据，例如地图。
...
染色配方是一种特殊的单 Item 合成配方，会将染料 ItemStack 的 `DataComponents#DYE` 合并为单个 `DataComponents#DYED_COLOR`，并应用到目标 ItemStack 以构造结果 ItemStack。目标 ItemStack 的所有其他组件也会复制到结果 ItemStack，与[转化合成][transmute]类似。例如：
...
- `target`：要应用染料的 [Ingredient][ingredient]。
- `dye`：用于为 ItemStack 染色的染料 [Ingredient][ingredient]。它们都必须具有 `DataComponents#DYE` [数据组件][datacomponent]。
- `result`：配方结果，即 [ItemStack 模板的 JSON 表示形式][itemjson]。
...
还有许多专门服务于单一用途（例如复制书、制作烟花）而非通用场景的合成配方。多数情况下，它们会根据输入 ItemStack 计算数值，并为输出设置数据组件。这些配方仍可配置，通常接收输入 Ingredient 与结果模板。例如：
...
这个用于制作烟花火箭的配方指定了生成结果所用的燃料、外壳与烟火之星 Ingredient。不过，它假定烟火之星 Ingredient 具有 `DataComponents#FIREWORK_EXPLOSION` 组件，否则不会添加爆炸效果。
...
- `minecraft:crafting_special_repairitem`：将两个损坏 Item 修复为一个。
...
- `ingredient`：配方的输入 [Ingredient][ingredient]。
- `result`：配方结果，即 [ItemStack 模板的 JSON 表示形式][itemjson]。
...
`type` 定义配方序列化器（`minecraft:stonecutting`）。ingredient 是一个 [Ingredient][ingredient]，result 是基础的 [ItemStack 模板 JSON][itemjson]。与合成配方一样，还可以选择指定 `group`，以便在配方书中分组。
...
请注意，单 Item 配方 builder 不支持真正的 ItemStack 结果，因此也不支持带数据组件的结果。不过，配方 codec 支持它们；如果需要此功能，就必须实现自定义 builder。

## 锻造

锻造台支持两种不同的配方序列化器：一种将输入转换为输出，并复制输入的组件（例如附魔）；另一种向输入应用组件。两者都使用 `minecraft:smithing` 配方类型，并需要三个输入，分别称为基础、模板和附加 Item。

### 转化锻造

此配方序列化器用于将两个输入 Item 转换成一个，并保留第一个输入的数据组件。原版主要将其用于下界合金装备，但这里可以使用任意 Item：
...
- `base`：配方的基础 [Ingredient][ingredient]，通常是某件装备。
- `template`：配方的模板 [Ingredient][ingredient]，通常是锻造模板。
- `addition`：配方的附加 [Ingredient][ingredient]，通常是某种材料，例如下界合金锭。
- `result`：配方结果，即 [ItemStack 模板的 JSON 表示形式][itemjson]。
...
- `base`：配方的基础 [Ingredient][ingredient]。所有原版用例都在此处使用 `minecraft:trimmable_armor` 标签。
- `template`：配方的模板 [Ingredient][ingredient]。所有原版用例都在此处使用锻造纹饰模板。
- `addition`：配方的附加 [Ingredient][ingredient]。所有原版用例都在此处使用 `minecraft:trim_materials` 标签。
- `pattern`：应用到基础 Ingredient 的纹饰图案。

值得注意的是，该配方序列化器没有 result 字段。这是因为它使用基础输入，并将模板与附加 Item“应用”到基础输入；也就是说，它根据其他输入设置基础输入的组件，并将操作结果用作配方结果。
