...
Minecraft 中有些位置提供了获取翻译键的 Helper Method。例如，Block 和 Item 都提供 `#getDescriptionId` 方法。对于 Item，不仅可以查询，还可以在需要时通过 `Item.Properties#overrideDescription` 更改。如果 Item 根据底层 [Data Component][datacomponent] 使用不同名称，可以在 `ItemStack` 上把 `CUSTOM_NAME` Data Component 设为所需的可翻译 Component，从而覆盖名称。`Item#getName` 还有一个接收 [`ItemStack`][itemstack] 参数的变体，用于设置 Item 的默认 Component。另一方面，`BlockItem` 通过调用 `Item.Properties#useBlockDescriptionPrefix` 设置 Description ID。`Identifier` 也提供 `toLanguageKey`，可以使用 Section 或 Registry 以及可选后缀构造翻译键。
