...
加载模块时，这一点更为重要。如果不同模块中存在两个同名包并且其中含有 class 文件，模组加载器会在启动时崩溃，因为模组模块会导出给游戏和其他模组。
...
执行某项任务往往有多种方法，例如注册对象、监听事件等。通常建议保持一致，对同一类任务只使用一种方法。这样可以提高可读性，并避免可能发生的异常交互或重复操作（例如事件监听器运行两次）。
