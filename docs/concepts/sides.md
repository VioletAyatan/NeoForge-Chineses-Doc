...
逻辑端主要关注 Minecraft 的内部程序结构。**逻辑服务端**是游戏逻辑运行的位置。时间和天气变化、Entity tick、Entity 生成等都在服务端运行。物品栏内容等各类数据也由服务端负责。另一方面，**逻辑客户端**负责显示所有需要显示的内容。Minecraft 将全部客户端代码隔离在 `net.minecraft.client` package 中，并在一个称为 Render Thread 的独立线程里运行它们；其余所有内容则视为通用代码（即客户端与服务端共用的代码）。
...
这两个场景也揭示了其中的主要问题：逻辑服务端能够使用你的代码，并不代表物理服务端也一定能够使用它。因此，你应始终使用专用服务端进行测试，以检查意外行为。由于客户端与服务端分离不正确而产生的 `NoClassDefFoundError` 和 `ClassNotFoundException`，是模组开发中最常见的错误之一。另一个常见错误是使用 static field，并从两个逻辑端访问它们；这尤其棘手，因为通常不会有任何迹象表明出了问题。

:::tip
如果需要将数据从一个端传输到另一个端，必须[发送数据包][networking]。
:::

在 NeoForge 代码库中，物理端由名为 `Dist` 的 enum 表示，逻辑端则由名为 `LogicalSide` 的 enum 表示。
...
这个 boolean 检查将是你最常用的端检查方式。查询 `Level` 对象上的这个 field，可以确定该 Level 所属的**逻辑端**：如果该 field 为 `true`，Level 正运行在逻辑客户端；如果为 `false`，Level 正运行在逻辑服务端。由此可知，物理服务端上的该 field 始终为 `false`，但不能反过来认为 `false` 就代表物理服务端，因为物理客户端内部的逻辑服务端（即单人游戏世界）上，该 field 同样可能为 `false`。
...
`FMLEnvironment#getDist()` 是 `Level#isClientSide()` 检查在**物理端**方面的对应方式。如果该 field 为 `Dist.CLIENT`，你正处于物理客户端；如果为 `Dist.DEDICATED_SERVER`，你正处于物理服务端。

#### `@Mod`

处理仅客户端可用的类时，检查物理环境十分重要。要分离只应在某一个物理端执行的代码，推荐做法是指定一个单独的 [`@Mod` annotation][mod]，并将 `dist` 参数设置为应加载该 mod class 的物理端：
