# NeoForge 26.1 官方文档中文翻译

> 原文：[NeoForged Documentation](https://docs.neoforged.net/docs/)  
> 对应版本：NeoForge 26.1  

## 技术术语处理原则

项目统一译名及易混概念见[项目术语表](GLOSSARY.md)。

- fenced code block 及其中的代码和注释保持原文，不进行翻译或改写。
- 行内代码、类名、方法名、字段名、包名、注册表键、资源路径、命令和配置键保持原文。
- `Block`、`Blocks`、`BlockState`、`BlockEntity`、`Item`、`Items`、`ItemStack`、`Entity`、`Level`、`Registry`、`Codec`、`Holder`、`ResourceLocation` 等编程对象不汉化。
- 普通说明文字使用简体中文；必要时采用“中文说明（英文术语）”形式，避免与实际 API 名称混淆。

## 目录

- [入门](gettingstarted/index.md)
- [核心概念](concepts/registries.md)
- [Blocks](blocks/index.md)
- [Items](items/index.md)
- [Entities](entities/index.md)
- [BlockEntity](blockentities/index.md)
- [资源](resources/index.md)
- [库存与传输](inventories/container.md)
- [数据存储](datastorage/nbt.md)
- [世界生成](worldgen/biomemodifier.md)
- [网络通信](networking/index.md)
- [渲染](rendering/feature.md)
- [高级主题](advanced/accesstransformers.md)
- [其他主题](misc/config.md)

## 许可与归属

原始文档版权所有 © 2026 NeoForged，并以 [MIT 许可证](https://github.com/neoforged/Documentation/blob/main/LICENSE)发布。本中文译文是基于原文制作的非官方翻译，保留原始文档的许可与归属信息。

NeoForge 文档并非 Minecraft 官方网站，未获得 Mojang 或 Microsoft 的批准，也不与其存在关联。
